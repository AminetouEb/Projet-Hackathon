...
function parseCo2Kg(raw: unknown): number {
  // Accepte string/nombre et securise les cas invalides pour l'affichage.
...
function normalizeToItem(
  ...
  // Compatibilite ascendante: supporte le format actuel + anciens formats API.
...
export class AppComponent {
  ...
  readonly suggestions = ['Latitude 7410', 'C2422HE Monitor'];
  ...
  detailRows(): { label: string; value: string }[] {
    // Genere dynamiquement les lignes "label / valeur" a partir de DETAIL_FIELDS.
    if (!this.item) return [];
  ...
  calculate(): void {
    // Nettoyage defensif pour accepter des copier-coller avec guillemets.
    ...
    this.http
      .post<CalculateResponse | LegacyMachineResponse | LegacyFrUsResponse>(
        environment.apiCalculateUrl,
        { machine: q },
      )
      ...
      .subscribe({
        next: (res) => {
          ...
          if (!it) {
            // Message explicite pour faciliter le diagnostic en demo/jury.
            this.apiError =
